...
@Component({
  ...
  template: `
    <div class="min-h-screen pt-24 pb-12 px-4 max-w-2xl mx-auto">
      ...
      <button (click)="store.setView('HOME')" class="mb-6 text-[#4A5D4A] hover:text-[#3A4A3A] transition-colors flex items-center gap-2 font-bold uppercase tracking-wide text-sm">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z" clip-rule="evenodd" /></svg>
        Seguir comprando
      </button>

      <h2 class="text-4xl font-black text-[#3A4A3A] mb-8 tracking-tight">Tu Carrito</h2>

      @if (store.cart().length === 0) {
        <div class="bg-white/80 rounded-[2rem] p-10 text-center shadow-lg border border-white">
          <div class="text-7xl mb-6">🍪</div>
          <p class="text-2xl text-[#3A4A3A] font-black mb-2">Tu carrito está vacío</p>
          <p class="text-[#4A5D4A] mb-8 text-lg">¡Corre a llenarlo de galletas!</p>
          <button (click)="store.setView('HOME')" class="bg-[#4A5D4A] text-white px-8 py-3 rounded-full font-bold shadow-lg hover:bg-[#3A4A3A] transition-all">Ver Menú</button>
        </div>
      } @else {
        <div class="space-y-4 mb-8">
          @for (item of store.cart(); track item.id) {
            <div class="bg-white rounded-2xl p-5 shadow-md flex justify-between items-start relative overflow-hidden group border border-gray-50">
              ...
              <div>
                <h3 class="font-black text-xl text-[#3A4A3A] mb-2">
                  <span *ngIf="item.quantity > 1" class="text-[#8FA67A] mr-1">{{item.quantity}}x</span>
                  {{ item.size === 1 ? 'Galleta Individual' : 'Caja x' + item.size }}
                </h3>
                <div class="flex flex-wrap gap-2">
                  @for (flavor of item.flavors; track $index) {
                    <span class="text-xs bg-[#e0eadc] text-[#3A4A3A] px-3 py-1.5 rounded-lg border border-[#c5d6be] font-bold">{{flavor.name}}</span>
                  }
                </div>
              </div>
              <div class="text-right flex flex-col justify-end h-full pt-8">
                <span class="font-black text-2xl text-[#3A4A3A]">{{ (item.price * item.quantity) | currency:'$':'symbol':'1.0-0' }}</span>
                @if (item.quantity > 1) {
          ...
          <div class="bg-white/60 p-6 rounded-2xl border border-white/50 backdrop-blur-sm space-y-2">
            ...
            <div class="flex justify-between items-center">
              <span class="text-2xl font-black text-[#3A4A3A]">Total</span>
              <span class="text-3xl font-black text-[#3A4A3A]">{{ store.cartTotal() | currency:'$':'symbol':'1.0-0' }}</span>
            </div>
          </div>
        </div>
        ...
        @if (store.storeConfig().isOpen) {
          <div class="bg-white rounded-[2rem] p-8 shadow-xl border border-gray-100">
            <h3 class="text-2xl font-black text-[#3A4A3A] mb-6 flex items-center gap-2">
            ...
            <form [formGroup]="checkoutForm" (ngSubmit)="submitOrder()" class="space-y-4">
              <div>
                ...
                <input type="text" formControlName="name" class="w-full bg-gray-50 border border-gray-200 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-[#8FA67A]/50" placeholder="Tu nombre">
              ...
              <div>
                ...
                <input type="tel" formControlName="phone" class="w-full bg-gray-50 border border-gray-200 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-[#8FA67A]/50" placeholder="300 123 4567">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" formControlName="address" class="w-full bg-gray-50 border border-gray-200 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-[#8FA67A]/50" placeholder="Calle 123 # 45-67">
                ...
                <div>
                  ...
                  <input type="text" formControlName="neighborhood" class="w-full bg-gray-50 border border-gray-200 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-[#8FA67A]/50" placeholder="Ej: Ciudad Jardín">
              ...
              <div class="bg-[#e0eadc] p-4 rounded-xl border border-[#c5d6be] flex items-start gap-3">
                <span class="text-2xl">ℹ️</span>
                <div>
                   <p class="font-bold text-[#3A4A3A] text-sm">Método de Pago</p>
                   <p class="text-xs text-[#4A5D4A]">Por ahora solo aceptamos transferencias. Te enviaremos los datos por WhatsApp al confirmar tu pedido.</p>
                </div>
              </div>

              <div>
                <label class="block text-sm font-bold text-gray-700 mb-1">Notas (Opcional)</label>
                <textarea formControlName="notes" rows="2" class="w-full bg-gray-50 border border-gray-200 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-[#8FA67A]/50" placeholder="Ej: Dejar en portería"></textarea>
              </div>
          ...
          <div class="bg-red-50 rounded-[2rem] p-10 text-center shadow-lg border border-red-100">
             <div class="text-7xl mb-6">🔒</div>
             <h3 class="text-3xl font-black text-[#3A4A3A] mb-4">Tienda Cerrada</h3>
             <p class="text-xl text-[#4A5D4A] mb-8 max-w-lg mx-auto">
               {{ store.storeConfig().closedMessage || 'Estamos descansando. Volvemos pronto.' }}
             </p>
             <button (click)="store.setView('HOME')" class="bg-[#4A5D4A] text-white px-8 py-3 rounded-full font-bold shadow-lg hover:bg-[#3A4A3A] transition-all">
                Volver al Menú
             </button>
          </div>
        }
