...
@Component({
  ...
  template: `
    ...
    <section id="inicio" class="video-header-section">
      <div class="video-container desktop-container">
        <!-- Desktop Video -->
        <video
          #headerVideo
          class="header-video"
          ...
          [muted]="true"
        >
          <source src="https://firebasestorage.googleapis.com/v0/b/biscoli-web.firebasestorage.app/o/Video%20header%20wide.mp4?alt=media&token=d82c55d5-0fee-4b99-9e0e-f1c70b1ffc1b" type="video/mp4">
        </video>
        ...
        <div class="video-cta-wrapper">
          <button (click)="scrollToSection('cajas')" class="video-cta-button">
      ...
      <div class="video-container mobile-container">
        <!-- Mobile Video -->
        <video
          #headerVideo
          class="header-video"
          ...
          [muted]="true"
        >
          <source src="https://firebasestorage.googleapis.com/v0/b/biscoli-web.firebasestorage.app/o/Video%20header%20vertical.mp4?alt=media&token=6514b2ed-39c2-4ce6-b139-48a754219817" type="video/mp4">
        </video>
    ...
    <section class="post-video-hero">
      ...
      <p class="text-xl md:text-2xl text-[#3A4A3A] font-bold max-w-3xl mb-4 leading-relaxed tracking-tight text-center px-6">
        ...
        <span class="text-[#5C2E35] font-black drop-shadow-sm">Sin Gluten. Sin Azúcar. Veganas.</span>
      </p>
      <div class="flex items-center gap-2 mb-6 bg-[#5C2E35]/10 backdrop-blur-sm px-5 py-2.5 rounded-full shadow-sm border border-[#5C2E35]/20">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-[#5C2E35]" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" /></svg>
        <span class="text-sm font-bold text-[#5C2E35] tracking-wide">Envíos solo en Cali</span>
      </div>
    </section>
    ...
    <section id="menu" class="py-16 px-4 max-w-7xl mx-auto mb-16 scroll-mt-24">
      <h2 class="text-5xl md:text-6xl text-center text-[#5C2E35] mb-4 font-black tracking-tighter">Nuestros <span class="text-[#5C2E35]">Sabores</span></h2>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6 md:gap-8">
        @for (flavor of store.flavors(); track flavor.id) {
          <div (click)="openModal(flavor)" class="group bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100 flex flex-col h-full cursor-pointer ring-1 ring-black/5 hover:ring-black/10 relative" [class.grayscale]="flavor.available === false" [class.opacity-90]="flavor.available === false">
            ...
            <div class="aspect-square w-full flex items-center justify-center relative overflow-hidden bg-[#FBF7F2]">
              @if (flavor.image) {
            ...
            <div class="p-5 md:p-6 flex-1 flex flex-col">
              ...
              <div class="mt-auto pt-4 border-t border-gray-100">
                <button class="w-full inline-flex items-center justify-center gap-2 text-[#5C2E35] border-2 border-[#5C2E35] px-6 py-2.5 rounded-full font-bold text-sm hover:bg-[#5C2E35] hover:text-white transition-all">
                  <span>Ver más</span>
    ...
    <section id="cajas" class="py-24 px-4 bg-[#3A4A3A] relative overflow-hidden scroll-mt-24">
      <div class="absolute top-0 w-full h-full left-0 opacity-20 pointer-events-none bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-[#5C2E35]/30 via-[#3A4A3A] to-[#3A4A3A]"></div>
      <div class="max-w-7xl mx-auto text-center relative z-10">
        <span class="text-[#C4735B] font-extrabold tracking-[0.2em] text-sm uppercase mb-3 block">La Experiencia Biscoli</span>
        <h2 class="text-5xl md:text-7xl mb-8 text-white font-black tracking-tight">Arma tu Caja</h2>
        <p class="mb-20 text-[#F5EDE2]/80 text-xl font-medium max-w-2xl mx-auto">Elige el tamaño perfecto para ti o para regalar. Personaliza cada espacio con tus sabores favoritos.</p>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-10 items-center">
          ...
          <div (click)="store.startBuildingBox(4)" class="group bg-gradient-to-br from-[#8FA67A]/20 to-[#3A4A3A]/80 backdrop-blur-md border border-[#8FA67A]/40 rounded-[2.5rem] p-12 relative transform md:scale-110 shadow-2xl cursor-pointer transition-all duration-300 hover:-translate-y-3 hover:shadow-[#8FA67A]/20 flex flex-col items-center z-10">
            <div class="absolute top-0 right-0 bg-[#C4735B] text-white text-xs font-black px-5 py-2 rounded-bl-2xl rounded-tr-[2.3rem] uppercase tracking-widest">Más Popular</div>
          ...
          <div (click)="store.startBuildingBox(6)" class="group bg-white/5 backdrop-blur-md border border-white/10 rounded-[2rem] p-8 hover:bg-white/10 cursor-pointer transition-all duration-300 hover:-translate-y-3 hover:shadow-2xl hover:shadow-black/50 flex flex-col items-center relative overflow-hidden">
            <div class="absolute top-0 right-0 bg-[#5C2E35] text-white text-xs font-black px-4 py-1.5 rounded-bl-2xl uppercase tracking-widest shadow-lg z-20">Envío Gratis</div>
    ...
    <section id="faq" class="py-24 px-4 bg-[#FBF7F2] text-[#3A4A3A] pb-32 border-t border-[#E8D5C0]">
       <div class="max-w-5xl mx-auto">
         <h2 class="text-5xl md:text-6xl text-center text-[#C4735B] mb-16 font-black tracking-tight">Preguntas <span class="text-[#C4735B]">Frecuentes</span></h2>
         <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
           <div class="bg-white rounded-3xl p-8 border border-[#E8D5C0] hover:border-[#C4735B]/30 hover:shadow-lg transition-all cursor-help">
             <h3 class="text-2xl font-bold text-[#3A4A3A] mb-3">¿Cuánto duran las galletas?</h3>
             <p class="text-[#3A4A3A]/70 text-lg leading-relaxed">Nuestras galletas son 100% naturales, sin conservantes. Recomendamos consumirlas dentro de los primeros 5 días. Si las refrigeras, pueden durar hasta 10 días.</p>
           </div>
           <div class="bg-white rounded-3xl p-8 border border-[#E8D5C0] hover:border-[#C4735B]/30 hover:shadow-lg transition-all cursor-help">
             <h3 class="text-2xl font-bold text-[#3A4A3A] mb-3">¿Cómo debo calentarlas?</h3>
             <p class="text-[#3A4A3A]/70 text-lg leading-relaxed">Para vivir la experiencia completa, caliéntalas entre 15 y 20 segundos en el microondas. ¡El chocolate se derretirá y quedarán increíbles!</p>
           </div>
           <div class="bg-white rounded-3xl p-8 border border-[#E8D5C0] hover:border-[#C4735B]/30 hover:shadow-lg transition-all cursor-help md:col-span-2">
             <h3 class="text-2xl font-bold text-[#3A4A3A] mb-3">¿Hacen envíos a todo el país?</h3>
             <p class="text-[#3A4A3A]/70 text-lg leading-relaxed">Por el momento solo realizamos envíos dentro de la ciudad de <strong>Cali</strong> para garantizar la frescura de nuestras galletas. ¡Esperamos expandirnos pronto a más ciudades!</p>
           </div>
         </div>
       </div>
    </section>
  ...
  styles: [`
    ...
    .video-container {
      ...
      overflow: hidden;
      max-height: 90vh;
      background-color: #3A4A3A; /* Placeholder bg while video loads */
    }
    /* Desktop container: reserve space with 16:9 aspect ratio */
    .desktop-container {
      aspect-ratio: 16 / 9;
      display: block;
    }
    ...
    .video-cta-button {
      ...
      align-items: center;
      gap: 0.5rem;
      background-color: #5C2E35;
      color: white;
      padding: 1rem 2.5rem;
      border-radius: 9999px;
      font-size: 1.25rem;
      font-weight: 900;
      box-shadow: 0 0 0 4px #F5EDE24d, 0 20px 60px rgba(0,0,0,0.4);
      border: none;
      cursor: pointer;
